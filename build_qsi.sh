...
# ==============================================================================
# TDEClamUI - Q4OS Installer (.qsi) Builder
# ==============================================================================
...
echo "=================================================="
echo " TDEClamUI - Q4OS .qsi Package Builder"
echo "=================================================="
...
# 2. Build Debian package if not found or rebuild requested
find_latest_deb() {
    local debs=("$SCRIPT_DIR"/tdeclamui_*.deb "$SCRIPT_DIR"/*.deb "$SCRIPT_DIR"/build/*.deb)
    for f in "${debs[@]}"; do
        if [ -f "$f" ]; then
            echo "$f"
            return 0
...
PACKAGE_NAME=$(dpkg-deb -f "$LATEST_DEB" Package 2>/dev/null || echo "tdeclamui")
APP_VERSION=$(dpkg-deb -f "$LATEST_DEB" Version 2>/dev/null || echo "1.0.1")
...
APPNAME_DESC="TDEClamUI Antivirus"
APP_ICON="tdeclamui"
PK_VERS="$APP_VERSION"
SETUP_TYPE="2"
